...
export async function deleteCategory(token: string, categoryId: number): Promise<void> {
  const response = await fetchWrapper(`/api/categories/${categoryId}`, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${token}` }
  });
  if (!response.ok) {
...
export async function updateCategory(token: string, categoryId: number, newName: string): Promise<SavedArticleCategory> {
  const response = await fetchWrapper(`/api/categories/${categoryId}`, {
    method: 'PUT',
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function updateArticleCategory(token: string, saved_article_id: number, newCategoryId: number | null): Promise<void> {
    const response = await fetchWrapper(`/api/saved/articles/${saved_article_id}/category`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
