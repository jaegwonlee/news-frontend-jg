...
import { getAllTopics, getPopularTopicsAll } from "@/lib/api"; // Import getAllTopics
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      ...
      try {
        ...
        const data = sortOrder === "popular" ? await getPopularTopicsAll() : await getAllTopics();
        setTopics(data);
      } catch (error) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-extrabold text-white">ROUND2</h1>
                ...
                <div className="relative bg-linear-to-r from-zinc-800 via-zinc-900 to-black p-8 rounded-3xl shadow-2xl group border-2 border-yellow-400/50 hover:border-yellow-400 transition-all duration-300 animate-glow-border-gold overflow-hidden shine-effect">
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
                    ...
                    <div className="grow text-center md:text-left">
                      <h2 className="text-3xl lg:text-4xl font-extrabold text-white mb-3 group-hover:text-yellow-300 transition-colors duration-300">
                        {topics[0].display_name}
                      </h2>
                      <p className="text-zinc-300 text-base lg:text-lg leading-relaxed line-clamp-2">
                        {topics[0].summary}
                      </p>
                    </div>
                ...
                <Link href={`/debate/${topic.id}`} key={topic.id}>
                  <div
                    className={`relative bg-zinc-800/80 backdrop-blur-sm p-5 rounded-2xl shadow-lg h-full flex flex-col transition-all duration-300 group border ${
                      isHolographic && theme
                        ? `${theme.borderColor} ${theme.hoverBorderColor}`
                        : "border-zinc-700/80 hover:border-red-500/50"
                      ...
                      {isHolographic && theme ? (
                        <div className="flex flex-col md:flex-row items-center gap-4 relative z-10">
                          <div className="shrink-0 text-center">
                            ...
                            <div className={`text-xs font-bold ${theme.rankColor}`}>RANK</div>
                          </div>
                          <div className="border-l-2 border-zinc-700/30 h-16 hidden md:block"></div>
                          <div className="grow text-center md:text-left">
                            <h2 className="text-xl font-bold text-white mb-2 leading-tight z-10">
                              {topic.display_name}
                            </h2>
                        ...
                        <div className="grow flex flex-col justify-center items-center text-center relative">
                          ...
                          <img
                            src="/avatars/blue--glove.svg"
                            alt="Blue Glove"
                            className="absolute left-0 top-1/2 -translate-y-1/2 w-24 h-24 opacity-20 group-hover:opacity-40 transition-all duration-300 group-hover:-translate-x-2"
                          />
                          <img
                            src="/avatars/red--glove.svg"
                            alt="Red Glove"
                            className="absolute right-0 top-1/2 -translate-y-1/2 w-24 h-24 opacity-20 group-hover:opacity-40 transition-all duration-300 group-hover:translate-x-2"
                          />
