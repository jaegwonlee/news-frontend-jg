...
export default function EditCategoryModal({ category, articlesInCategory, uncategorizedArticles, onClose, onSave }: EditCategoryModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-zinc-900 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="p-6 flex justify-between items-center border-b border-zinc-700">
          <h2 className="text-2xl font-bold text-white">카테고리 수정</h2>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white mb-3">기사 추가하기 (미분류)</h3>
              <div className="bg-zinc-800/50 rounded-lg p-3 border border-zinc-700 h-64 overflow-y-auto space-y-2">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white mb-3">현재 포함된 기사 ({articlesInCategory.length})</h3>
            <div className="bg-zinc-800/50 rounded-lg p-3 border border-zinc-700 h-[20.5rem] overflow-y-auto space-y-2">
        ...
        <footer className="p-6 flex justify-end gap-4 border-t border-zinc-700">
          ...
          <button onClick={handleSave} disabled={isSaving || !categoryName.trim()} className="px-6 py-3 bg-red-600 text-white rounded-md font-semibold hover:bg-red-700 transition-colors disabled:bg-zinc-500 disabled:cursor-not-allowed">
